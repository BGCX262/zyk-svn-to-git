...
class Vow_Lucene_Analyzer extends Zend_Search_Lucene_Analysis_Analyzer_Common
{
    ...
    //public $mainDicFile = 'httpcws_dict_utf8.txt';
    ...
    public function reset()
    {
        ...
        if( $this->_input != '')
        {
            if( preg_match("/^utf/", $this->_encoding) ) {
                $this->sourceString = iconv('utf-8//ignore', 'ucs-2', $this->_input);
            }
            else if( preg_match("/^gb/", $this->_encoding) ) {
                $this->sourceString = iconv('utf-8', 'ucs-2', iconv('gb18030', 'utf-8//ignore', $this->_input));
            }
            else if( preg_match("/^big/", $this->_encoding) ) {
                $this->sourceString = iconv('utf-8', 'ucs-2', iconv('big5', 'utf-8', $this->_input));
            }
        }
        $this->StartAnalysis();
       	$this->_input = $this->GetFinallyResult(' ');
    }
    ...
    public function __construct($source_charset='utf-8', $target_charset='utf-8', $load_all=true)
    {
    ...
    */
     ...
     */
    public function GetWordInfos($word)
    {
        $rearr = '';
        if( strlen($word) < 4 )
        {
     ...
     * @return bool
     */
    public public function StartAnalysis($optimize=true)
    {
        if( !$this->isLoadDic )
        {
            $this->LoadDict();
     ...
     * @return bool
     */
    public function DeepAnalysis( &$str, $ctype, $spos, $optimize=true )
    {
	    //echo '<div style="border-bottom:2px solid red">'.iconv('ucs-2', 'utf-8', $str).'</div>';
        //中文句子
        if( $ctype==1 )
        {
            $slen = strlen($str);
     ...
     * @return void
     */
    public function DeepAnalysisChinese( &$str, $lastec, $spos, $slen, $optimize=true )
    {
        ...
        for($i=$slen-1; $i>0; $i--)
        {
            ...
            {
                $tmparr[] = $nc;
				//echo '<span style="color:#546541">'.iconv('ucs-2', 'utf-8', $nc).'</span><br />';
                $i = 0;
                break;
            }
            if( $this->IsWordEnd($nc) )
            {
                ...
                for($k=12; $k>1; $k=$k-2)
                {
                    ...
                    {
                        $tmparr[] = $w.$nc;
						//echo '<span style="color:red">'.iconv('ucs-2', 'utf-8', $w.$nc).'</span><br />';
                        $i = $i - $k;
                        $isok = true;
                        break;
                    }
                ...
                {
                   $tmparr[] = $nc;
				   //echo '<span style="color:blue">'.iconv('ucs-2', 'utf-8', $nc).'</span><br />';
				   //echo iconv('ucs-2', 'utf-8', $nc).'<br/>';
                }
            }
            else
            {
               $tmparr[] = $nc;
			   //echo '<span style="color:#415414">'.iconv('ucs-2', 'utf-8', $nc).'</span><br />';
               $i = $i - 1;
            }
        }
    ...
    */
    ...
    public function OptimizeResult( &$smarr, $spos )
    {
        $newarr = array();
        $prePos = $spos - 1;
        $arlen = count($smarr);
    ...
    * @return void
    */
    public function SortFinallyResult()
    {
    	  $newarr = array();
        $i = 0;
     ...
     * return string
     */
     public function OutStringEncoding( &$str )
     {
     ...
     * @return int
     */
     public function SourceResultCharset()
     {
